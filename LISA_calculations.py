...
def g_n(n,m0,m1,Porb,e):
    ...
    return gn
def fdotn(m0,m1,Porb,e,n,z):
    # Equation (4) from Willems et al. 2007
    F = (1 + 73./24.*pow(e,2.) + 37./96.*pow(e,4.))*pow(1-e*e,-7./2.)
    nu = 1./Porb
    Mc = pow(m0*m1,3./5.)/pow(m0+m1,1./5.)
    fdotn = 1./(1.+z) * n * 96./(10.*np.pi)*pow(Mc,5./3.)*pow(2*np.pi*nu,11./3.)*F*G**(5./3.)/c**5.
    return fdotn
...
def hn(m1,m2,a,e,d,n):
    ...
    h_n = 1./(np.pi*D)*(2.*1./c**3.0 * (G*Mchirp*Msun)**(5./3.) / (3.*np.pi**(1./3.)*(1.+z)**(1./3.)*(n*f_orb/(1.+z))**(1./3.)) * (2./n)**(2./3.) * gn/F)**0.5
    #print 'h_n', h_n, h_n/np.sqrt(G)
    return h_n
...
def SNR_calculator(m0,m1,a,e,d):
	...
	z = zAtLuminosityDistance(d) 
	
	#d = d*1.e6*3.086e16     # CONVERT D TO METERS
	SNR_tot_sq = 0
	### IMPORT LISA NOISE CURVE
	data2 = np.loadtxt('characteristic_noise_strain.dat')
	fn = data2[:,0]
	hn_lisa = data2[:,1]
	###########################
	Tobs = 10.0*3.15e7
	...
	#	SNR = np.sqrt(16./5.*pow(ho,2.)*Tobs/pow(h_lisa,2.))
	#if e >= 0.0001:
	...
	#		SNR = np.sqrt(16./5.*SNR_tot_sq)
	for n in range(1,100):
		h_lisa = np.interp(f_orb*n,fn[:],hn_lisa[:])
		strain = hn(m0,m1,a,e,d,n)*np.sqrt(fdotn_2(m0,m1,Porb,e,n,z)*Tobs/f_orb)
		#print n, strain, h_lisa
		SNR_n_sq = strain**2./h_lisa**2.
		SNR_tot_sq = SNR_tot_sq + SNR_n_sq
	SNR = np.sqrt(2.)*np.sqrt(16./5.*SNR_tot_sq)
...
def t_inspiral_2(a0,e0,m1,m2,t_flag=0, array=0, LIGO=0,ecc_at_fGW_01=0):
	"""
	...
	if LIGO=1, integrate ALL the way to inspiral (meaning take very small timesteps at end)

	if ecc_at_fGW_01 = 1, output eccentricity at fGW_peak = 10^{-2} Hz
	"""
	M1 = m1*1.989e30
        M2 = m2*1.989e30
        G = 6.67408e-11
	...
	if ecc_at_fGW_01 == 1:
 		# find inspiral time to forb=10^{-2} Hz and use that as t_max (so you don't take too big of steps at beginning)
                a_2 = (G*(M1+M2)/(4.*np.pi**2.)*(1.e2)**2.)**(1./3.)/1.5e11 # a at f_orb = 10^-2 in AU
                t_2, a_f_2, e_2 = inspiral_time_peters(a0,e0,m1,m2,af=a_2)
		t_max = t_2*1.e9*1.1
		t_start = 1.0
		#print 't_max =', t_2*1.e9
	...
	for i in range(len(t)):
		...
		fGW_peak = np.sqrt(G*(M1+M2))/np.pi*(1+e)**1.1954/(a*(1-e**2.))**1.5
		if fGW_peak >= 0.01 and ecc_at_fGW_01_flag == 0 and a > 0 and e > 0:
			ecc_at_fGW_01_value = e
			a_at_fGW_01_value = a/1.496e11
			ecc_at_fGW_01_flag = 1
	...
	if t_flag == 0 and array == 0 and ecc_at_fGW_01 == 0:
		return t_FINAL
	if t_flag > 0 and array == 0 and ecc_at_fGW_01 == 0:
		return SMA, ECCENTRICITY
	if array == 1 and ecc_at_fGW_01 == 0:
		return t_array, a_array, ecc_array
	if ecc_at_fGW_01 == 1:
		return ecc_at_fGW_01_value, a_at_fGW_01_value
