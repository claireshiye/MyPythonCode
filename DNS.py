...
def find_formationtime_DNS(ids, filepath): ##Find formation time of merging BNSs
...
def find_formationtime_NSBH(ids, filepath): ##Find formation time of merging NSBHs
	snaps=np.sort(glob(filepath+'*.snap*.dat.gz'))
	#t_conv=dyn.conv('t', filestring+'.conv.sh')
	tform=-100; snapno=-100
	for m in range(0, len(snaps)):
		...
		with gzip.open(snaps[m], 'r') as fsnap:
			...
			for line in fsnap:
				datasnap=line.split()
				if ((int(datasnap[10])==ids[0] and int(datasnap[11])==ids[1]) or (int(datasnap[10])==ids[1] and int(datasnap[11])==ids[0])) and ((int(datasnap[17])==13 and int(datasnap[18])==14) or (int(datasnap[17])==14 and int(datasnap[18])==13)):
					tform=dyn.get_time(snaps[m])
					#tform=tform*t_conv  ##in Myr
					snapno=m
					check=1
...
def find_all_mergers_DNS(pathlist, start, end, typeflag):   ##DNSs both in the cluster and escaped
	...
	#np.savetxt(savepath+'/newruns/finaldata/num_merger_'+typeflag+'_extreme.dat', np.c_[Models, Ncoll, Ncoll2, Ncoll3, Ncoll4, Nsemerge, Nesc, Nescmerge, status], fmt='%d %d %d %d %d %d %d %d %d', header='1.Model 2.Ncoll 3.Ncoll2 4.Ncoll3 5.Ncoll4 6.Nsemerge 7.Nesc 8.Nescmerge 9.Status(1-done; 2&3-dissolved)', delimiter='', comments='#')
...
def find_all_mergers_NSBH(pathlist, start, end):   ##NSBHs both in the cluster and escaped
	sourcedir=np.genfromtxt(pathlist, dtype=str)
	status=map(int, sourcedir[:,1]); sourcedir=sourcedir[:,0]
	...
	r_merge=[]; r_coll=[]
	tform_semerge=[]; snapno_semerge=[]
	primordial_bin=[]; dynamics_ejection=[]
	...
	for i in range(start, end):
		...
		for j in range(len(colldata)):
			line=colldata[j].split()
			if int(line[2])==2:  ##Single-single star collision
				if (int(line[11])==13 and int(line[12])==14) or (int(line[11])==14 and int(line[12])==13):
					model_coll.append(i); status_coll.append(status[i])
					ncoll2+=1
					idcoll.append(int(line[3])); id0coll.append(int(line[5])); id1coll.append(int(line[7])); id2coll.append(-100); id3coll.append(-100)
					typecoll.append(int(line[2]))
			...
			if int(line[2])==3:  ##Binary-single star collision
				if len(line)==16:  ##Three stars collision
					if (int(line[13])==14 and (int(line[14])==13 or int(line[15])==13)) or (int(line[14])==14 and (int(line[13])==13 or int(line[15])==13)) or (int(line[15])==14 and (int(line[13])==13 or int(line[14])==13)):
						print 'types=', int(line[13]); int(line[14]), int(line[15])
						model_coll.append(i); status_coll.append(status[i])
						ncoll3+=1
						idcoll.append(int(line[3])); id0coll.append(int(line[5])); id1coll.append(int(line[7])); id2coll.append(int(line[9])); id3coll.append(-100)
						typecoll.append(int(line[2]))
			...
			if int(line[2])==4:  ##Binary-binary star collision
				if len(line)==19: ##Four stars collision
					if (int(line[15])==14 and (int(line[16])==13 or int(line[17])==13 or int(line[18])==13)) or (int(line[16])==14 and (int(line[15])==13 or int(line[17])==13 or int(line[18])==13)) or (int(line[17])==14 and (int(line[16])==13 or int(line[15])==13 or int(line[18])==13)) or (int(line[18])==14 and (int(line[16])==13 or int(line[17])==13 or int(line[15])==13)):
						print 'types=', int(line[15]); int(line[16]), int(line[17]), int(line[18])
						model_coll.append(i); status_coll.append(status[i])
						ncoll4+=1
						idcoll.append(int(line[3])); id0coll.append(int(line[5])); id1coll.append(int(line[7])); id2coll.append(int(line[9])); id3coll.append(int(line[11]))
						typecoll.append(int(line[2]))
		...
		for k in range(len(semergedata)):
			line=semergedata[k].split()
			if int(line[1])<3:
				if (int(line[-1])==13 and int(line[-2])==14) or (int(line[-1])==14 and int(line[-2])==13):
					print 'types=', int(line[-1]), int(line[-2])
					model_merge.append(i); status_merge.append(status[i])
					nsemerge+=1
					timesemerge.append(float(line[0])); timesemerge_myr.append(t_conv*float(line[0]))
					idsemerge.append(int(line[2])); id0merge.append(int(line[4])); id1merge.append(int(line[6]))
					mf_merge.append(float(line[3])); m0_merge.append(float(line[5])); m1_merge.append(float(line[7]))
					r_merge.append(float(line[8]))
					#tf_semerge, sno_semerge, pribin, dyn_eject=find_formationtime_NSBH_new([int(line[4]), int(line[6])], filestr)
					#if tf_semerge!=-100:
					#	tform_semerge.append(tf_semerge*t_conv); snapno_semerge.append(sno_semerge)
					#else:
					#	tform_semerge.append(t_conv*float(line[0])); snapno_semerge.append(-100)
					#primordial_bin.append(pribin); dynamics_ejection.append(dyn_eject)
		...
		fescnsbh=open(savepath+'/newruns/Esc_NSBH_maingrid.dat', 'a+', 0)
		#fescnsbh.write('#1.Model 2.Time_esc(code) 3.Time_esc(Myr) 4.T_insp(Myr) 5.M0 6.M1 7.ID0 8.ID1 9.A 10.ECC 11.Tform(Myr) 12.Primordial 13.Dynamically_Ejected?\n')
		####For mergers that happen outside of the clusters####
		escfile=filestr+'.esc.dat'
		with open(escfile, 'r') as fesc:
			next(fesc)
			for line in fesc:
				dataesc=line.split()
				if int(dataesc[14])==1:
					if (int(dataesc[22])==13 and int(dataesc[23])==14) or (int(dataesc[22])==14 and int(dataesc[23])==13):
						print 'types=', int(dataesc[22]), int(dataesc[23])
						nesc+=1
						#print float(dataesc[19]), float(dataesc[20]), float(dataesc[15]), float(dataesc[16])
						t_inspiral=gwcalc.t_inspiral_2(float(dataesc[19]), float(dataesc[20]), float(dataesc[15]), float(dataesc[16]), 0, 0, 0, 1100)/10**6 ##in Myr
						tesc=float(dataesc[1])*t_conv  ##In Myr
						...
						timeesc_myr=float(dataesc[1])*t_conv; tins=t_inspiral
						timeesc=float(dataesc[1]); m0=float(dataesc[15]); m1=float(dataesc[16]); id0=int(dataesc[17]); id1=int(dataesc[18]); a=float(dataesc[19]); ecc=float(dataesc[20])
						tf_esc, sno_esc, pribin, dyn_eject=find_formationtime_NSBH_new([int(dataesc[17]), int(dataesc[18])], filestr)
						if tf_esc!=-100:
							tform_esc=tf_esc*t_conv; snapno_esc=sno_esc
						else:
							tform_esc=float(dataesc[1])*t_conv; snapno_esc=-100

						fescnsbh.write('%d %f %f %f %f %f %d %d %f %f %f %d %d %d\n'%(model_esc, timeesc, timeesc_myr, tins, m0, m1, id0, id1, a, ecc, tform_esc, pribin, dyn_eject, status[i]))
						...
						if t_inspiral+tesc<=14000.:
							...
							#timeesc_myr=float(dataesc[1])*t_conv; tins=t_inspiral
							#timeesc=float(dataesc[1]); m0=float(dataesc[15]); m1=float(dataesc[16]); id0=int(dataesc[17]); id1=int(dataesc[18]); a=float(dataesc[19]); ecc=float(dataesc[20])
							#tf_esc, sno_esc=find_formationtime_NSBH([int(dataesc[17]), int(dataesc[18])], filestr)
							#if tf_esc!=-100:
							#	tform_esc=tf_esc*t_conv; snapno_esc=sno_esc
							#else:
							#	tform_esc=float(dataesc[1])*t_conv; snapno_esc=-100
	...
	##Output files
	np.savetxt(savepath+'/newruns/finaldata/GWcap_NSBH_maingrid.dat', np.c_[model_coll, timecoll, timecoll_myr, typecoll, idcoll, id0coll, id1coll, id2coll, id3coll, mf_coll, m0_coll, m1_coll, m2_coll, m3_coll, r_coll, k0, k1, k2, k3, status_coll], fmt='%d %f %f %d %d %d %d %d %d %f %f %f %f %f %f %d %d %d %d %d', delimiter='', header='1.Model 2.Time(code) 3.Time(Myr) 4.Type 5.IDM 6.ID0 7.ID1 8.ID2 9.ID3 10.MM 11.M0 12.M1 13.M2 14.M3 15.R 16.K0 17.K1 18.K2 19.K3 20.Status(1-done; 2&3-dissolved)', comments='#')

	#np.savetxt(savepath+'/newruns/Incluster_NSBH_maingrid.dat', np.c_[model_merge, timesemerge, timesemerge_myr, idsemerge, id0merge, id1merge, mf_merge, m0_merge, m1_merge, r_merge, tform_semerge, primordial_bin, dynamics_ejection, status_merge], fmt='%d %f %f %d %d %d %f %f %f %f %f %d %d %d', delimiter='', header='1.Model 2.Time(code) 3.Time(Myr) 4.IDM 5.ID0 6.ID1 7.MM 8.M0 9.M1 10.R 11.Tform(Myr) 12.Primordial 13.Dynamics? 14.Status(1-done; 2&3-dissolved)', comments='#')


	np.savetxt(savepath+'/newruns/finaldata/nummerger_NSBH_maingrid.dat', np.c_[Models, Ncoll, Ncoll2, Ncoll3, Ncoll4, Nsemerge, Nesc, Nescmerge, status], fmt='%d %d %d %d %d %d %d %d %d', header='1.Model 2.Ncoll 3.Ncoll2 4.Ncoll3 5.Ncoll4 6.Nsemerge 7.Nesc 8.Nescmerge 9.Status(1-done; 2&3-dissolved)', delimiter='', comments='#')



def find_all_mergers_BBH(pathlist, start, end):   ##BBHs both in the cluster and escaped
	sourcedir=np.genfromtxt(pathlist, dtype=str)
	...
	for i in range(start, end):
		...
		for j in range(len(colldata)):
			line=colldata[j].split()
			if int(line[2])==2:  ##Single-single star collision
				if int(line[11])==14 and int(line[12])==14:
					model_coll.append(i)
					ncoll2+=1
					idcoll.append(int(line[3])); id0coll.append(int(line[5])); id1coll.append(int(line[7])); id2coll.append(-100); id3coll.append(-100)
					typecoll.append(int(line[2]))
			...
			if int(line[2])==3:  ##Binary-single star collision
				if len(line)==16:  ##Three stars collision
					if (int(line[13])==14 and int(line[14])==14) or (int(line[14])==14 and int(line[15])==14) or (int(line[13])==14 and int(line[15])==14):
						model_coll.append(i)
						ncoll3+=1
						idcoll.append(int(line[3])); id0coll.append(int(line[5])); id1coll.append(int(line[7])); id2coll.append(int(line[9])); id3coll.append(-100)
						typecoll.append(int(line[2]))
			...
			if int(line[2])==4:  ##Binary-binary star collision
				if len(line)==19: ##Four stars collision
					if (int(line[15])==14 and int(line[16])==14) or (int(line[16])==14 and int(line[17])==14) or (int(line[17])==14 and int(line[18])==14) or (int(line[15])==14 and int(line[17])==14) or (int(line[15])==14 and int(line[18])==14) or (int(line[16])==14 and int(line[18])==14):
						model_coll.append(i)
						ncoll4+=1
						idcoll.append(int(line[3])); id0coll.append(int(line[5])); id1coll.append(int(line[7])); id2coll.append(int(line[9])); id3coll.append(int(line[11]))
						typecoll.append(int(line[2]))
		...
		for k in range(len(semergedata)):
			line=semergedata[k].split()
			if int(line[1])<3:
				if int(line[-1])==14 and int(line[-2])==14:
					model_merge.append(i)
					nsemerge+=1
					timesemerge.append(float(line[0])); timesemerge_myr.append(t_conv*float(line[0]))
					idsemerge.append(int(line[2])); id0merge.append(int(line[4])); id1merge.append(int(line[6]))
					mf_merge.append(float(line[3])); m0_merge.append(float(line[5])); m1_merge.append(float(line[7]))
					r_merge.append(float(line[8]))
					tf_semerge, sno_semerge, pribin, dyn_eject=find_formationtime_BBH([int(line[4]), int(line[6])], filestr)
					if tf_semerge!=-100:
						tform_semerge.append(tf_semerge*t_conv); snapno_semerge.append(sno_semerge)
					else:
						tform_semerge.append(t_conv*float(line[0])); snapno_semerge.append(-100)
		...
		#fescbbh=open(savepath+'/newruns/Esc_BBH_nondissolved_10.dat', 'a+', 0)
		#fescbbh.write('#1.Model 2.Time_esc(code) 3.Time_esc(Myr) 4.T_insp(Myr) 5.M0 6.M1 7.ID0 8.ID1 9.A 10.ECC 11.Tform(Myr) 12.Primordial? 13.Dynamically_Ejected?\n')
		####For mergers that happen outside of the clusters####
		escfile=filestr+'.esc.dat'
		with open(escfile, 'r') as fesc:
			next(fesc)
			for line in fesc:
				dataesc=line.split()
				if int(dataesc[14])==1:
					if int(dataesc[22])==14 and int(dataesc[23])==14:
						nesc+=1
						#print float(dataesc[19]), float(dataesc[20]), float(dataesc[15]), float(dataesc[16])
						#t_inspiral=gwcalc.t_inspiral_2(float(dataesc[19]), float(dataesc[20]), float(dataesc[15]), float(dataesc[16]), 0, 0, 0, 1100)/10**6 ##in Myr
						#t_inspiral=14000.
						...
						tesc=float(dataesc[1])*t_conv  ##In Myr
						model_esc=i
						timeesc_myr=float(dataesc[1])*t_conv; tins=t_inspiral#tins=-100
						timeesc=float(dataesc[1]); m0=float(dataesc[15]); m1=float(dataesc[16]); id0=int(dataesc[17]); id1=int(dataesc[18]); a=float(dataesc[19]); ecc=float(dataesc[20])
						#tf_esc, sno_esc, pribin, dyn_eject=find_formationtime_BBH([int(dataesc[17]), int(dataesc[18])], filestr)
						#if tf_esc!=-100:
						#	tform_esc=tf_esc*t_conv; snapno_esc=sno_esc
						#else:
						#	tform_esc=float(dataesc[1])*t_conv; snapno_esc=-100

						#fescbbh.write('%d %f %f %f %f %f %d %d %f %f %f %d %d\n'%(model_esc, timeesc, timeesc_myr, tins, m0, m1, id0, id1, a, ecc, tform_esc, pribin, dyn_eject))
						#print model_esc
	...
	##Output files
	#np.savetxt(savepath+'/newruns/GWcap_BBH_nondissolved.dat', np.c_[model_coll, timecoll, timecoll_myr, typecoll, idcoll, id0coll, id1coll, id2coll, id3coll, mf_coll, m0_coll, m1_coll, m2_coll, m3_coll, r_coll, k0, k1, k2, k3], fmt='%d %f %f %d %d %d %d %d %d %f %f %f %f %f %f %d %d %d %d', delimiter='', header='1.Model 2.Time(code) 3.Time(Myr) 4.Type 5.IDM 6.ID0 7.ID1 8.ID2 9.ID3 10.MM 11.M0 12.M1 13.M2 14.M3 15.R 16.K0 17.K1 18.K2 19.K3', comments='#')

	np.savetxt(savepath+'/newruns/Incluster_BBH_nondissolved_5.dat', np.c_[model_merge, timesemerge, timesemerge_myr, idsemerge, id0merge, id1merge, mf_merge, m0_merge, m1_merge, r_merge, tform_semerge, primordial_bin, dynamics_ejection], fmt='%d %f %f %d %d %d %f %f %f %f %f %d %d', delimiter='', header='1.Model 2.Time(code) 3.Time(Myr) 4.IDM 5.ID0 6.ID1 7.MM 8.M0 9.M1 10.R 11.Tform(Myr) 12.Primordial 13.Dynamics?', comments='#')
	...
	#np.savetxt(savepath+'/newruns/nummerger_BBH_nondissolved.dat', np.c_[Models, Ncoll, Ncoll2, Ncoll3, Ncoll4, Nsemerge, Nesc, Nescmerge], fmt='%d %d %d %d %d %d %d %d', header='1.Model 2.Ncoll 3.Ncoll2 4.Ncoll3 5.Ncoll4 6.Nsemerge 7.Nesc 8.Nescmerge', delimiter='', comments='#')
...
def find_DNS_NSBH_Unique(datfile):
	...
	funique=open('/projects/b1095/syr904/projects/SGRB/newruns/finaldata/DNS_NSBH_Unique_9to12Gyr_maingrid.dat', 'a+', 0)
...
def num_analysis(filepath):
	...
	data_names=['dns_esc', 'dns_in', 'dns_cap', 'nsbh_esc', 'nsbh_in', 'nsbh_cap']
	data_objects=[dns_esc, dns_in, dns_cap, nsbh_esc, nsbh_in, nsbh_cap]
	...
	nesc_dns=0; nesc_nsbh=0
	pribinary=[[], [], [], [], [], []]; dynamic=[[], [], [], [], [], []]
	for j in range(len(time_esc_dns)):
		if time_esc_dns[j]+time_inspiral_dns[j]<=14000.0: 
			nesc_dns+=1
			pribinary[0].append(data_objects[0][:,11][j])
			dynamic[0].append(data_objects[0][:,12][j])
	for k in range(len(time_esc_nsbh)):
		if time_esc_nsbh[k]+time_inspiral_nsbh[k]<=14000.0: 
			nesc_nsbh+=1
			pribinary[3].append(data_objects[3][:,11][k])
			dynamic[3].append(data_objects[3][:,12][k])
	...
	for i in range(len(data_objects)):
		models.append(data_objects[i][:,0])

		if i<3: num_dns.append(len(data_objects[i][:,0]))
		else: num_nsbh.append(len(data_objects[i][:,0]))

		if i==2 or i==5:
			pribinary[i]=list(np.zeros(len(data_objects[i][:,0])))
			dynamic[i]=list(np.ones(len(data_objects[i][:,0])))
		if i==1 or i==4:
			pribinary[i]=data_objects[i][:,11]
			dynamic[i]=data_objects[i][:,12]
	...
	for i in range(len(data_objects)):
		...
		num_dyn.append(np.sum(dynamic[i]))
	...
	num_dns[0]=nesc_dns; num_nsbh[0]=nesc_nsbh


	num_dns=np.array(num_dns); num_nsbh=np.array(num_nsbh)	
	ntot_dns=float(sum(num_dns)); ntot_nsbh=float(sum(num_nsbh))

	frac_dns=num_dns/ntot_dns; frac_nsbh=num_nsbh/ntot_nsbh

	print num_dns, num_nsbh
	print frac_dns, frac_nsbh
	print ntot_dns, ntot_nsbh
	print num_pri
	print num_dyn
...
def psr_analysis(filepath):
	...
	data_names=['dns_esc', 'dns_in', 'nsbh_esc', 'nsbh_in']
	...
	for i in range(len(data_objects)):
		...
		for j in range(len(B0)):
			if B0[j]!=0 and B0[j]!=-100 and tmerge[j]<=14000.:
				...
				if psr0=="yes" and P0_new>0.03: npsr0[j]=2

			if B1[j]!=0 and B1[j]!=-100 and tmerge[j]<=14000.:
				...
				if psr1=="yes" and P1[j]>0.03: npsr1[j]=2
		...
		print nbmsp, nbpsr, npsrmsp, nsmsp, nspsr
...
def get_id_formationchannel(pathlist, sourcepath):
	...
	escdns=np.genfromtxt(sourcepath+'Esc_DNS_maingrid.dat')
	indns=np.genfromtxt(sourcepath+'Incluster_DNS_maingrid.dat')
	gwdns=np.genfromtxt(sourcepath+'GWcap_DNS_maingrid.dat')
	escnsbh=np.genfromtxt(sourcepath+'Esc_NSBH_maingrid.dat')
	innsbh=np.genfromtxt(sourcepath+'Incluster_NSBH_maingrid.dat')
	...
	for i in range(2):
		...
		for j in range(len(esc_model)):
			...
			with open(filestring+'esc.dat', 'r') as fesc:
				next(fesc)
				for line in fesc:
					dataesc=line.split()
					if int(dataesc[14])==1 and int(dataesc[17])==int(esc_id0[j]) and int(dataesc[18])==int(esc_id1[j]):
							esc_fc0[i].append(int(dataesc[47])); esc_fc1[i].append(int(dataesc[48]))
