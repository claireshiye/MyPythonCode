...
def history_maker(id0, id1, path, filestring, snapno, time_i):
	ID_FLAG = 0
	MTflag = 1	
	id0_str = str(id0)
	#f = open('history/highB/'+id0_str+'_pp.dat','w')
	#sys.stdout = f
        units=scripts.read_units(path+'/'+filestring)
	...
	for j in range(snapno,0,Delta):
		...
		f2 = gzip.open(path+'/'+filestring+'.snap'+snapno+'.dat.gz','r')
		lines = f2.readlines()
		line = lines[0]
		parsed = line.split('[')
		parsed = parsed[0]
		...
		###############################
		        ...
		        if j-Delta < 10:
                        ...
                        f2_f = gzip.open(path+'/'+filestring+'.snap'+snapno_f+'.dat.gz','r')
			...
			f2_f = gzip.open(path+'/'+filestring+'.snap'+snapno_max_str+'.dat.gz','r')
                        lines2 = f2_f.readlines()
                        line2 = lines2[0]
                        parsed = line2.split('[')
                        parsed = parsed[0]
						...
						if VAR == id0:
							for l in range(len(h[id0]['binint']['binint'][i]['interaction']['output'][m]['ids'])):
								if float(h[id0]['binint']['binint'][i]['interaction']['output'][m]['ids'][l]) == id1:
									############  TRIPLE STUFF   ######################
									for p in range(len(h[id0]['binint']['binint'][i]['interaction']['output'][m]['m'])):
										if p == 2:	# DETERMINE IF A TRIPLE IS FORMED
											TRIPLEflag = 1
									if TRIPLEflag == 1:
										TRIPLEflag = 0
										print timeI, 3, -1, -1, -1, -1,
							...
							if len(h[id0]['binint']['binint'][i]['interaction']['output'][m]['ids'])==2:
									#for k in range(0,len(h[id0]['binint']['binint'][i]['interaction']['output'][m]['m'])):
                                                                                #if k <= 1:
                                                        	print h[id0]['binint']['binint'][i]['interaction']['output'][m]['ids'][0], h[id0]['binint']['binint'][i]['interaction']['output'][m]['ids'][1], -1, -1, h[id0]['binint']['binint'][i]['interaction']['output'][m]['m'][0], h[id0]['binint']['binint'][i]['interaction']['output'][m]['m'][1],
							...
							if len(h[id0]['binint']['binint'][i]['interaction']['output'][m]['ids'])==1:
								print h[id0]['binint']['binint'][i]['interaction']['output'][m]['ids'][0], -1, -1, -1, h[id0]['binint']['binint'][i]['interaction']['output'][m]['m'][0], -1, -1, -1, MTflag, 0
		...
		for i in range(2,len(lines)):
			...
			MTflag = 0
            		...
            		if float(data[7]) == 0.:
                		M0 = float(data[1])
                		if float(data[0]) == id0:
                    			print time, 0, int(data[0]), -1, int(data[14]), -1, M0, -1, -1, -1, -1, snapno
                    			break
			...
			if float(data[7]) == 1.:
				...
				if float(data[10]) == id0:
					...
					timearr.append(time)
					ID_FLAG = 0
					print time, 0, int(data[10]), int(data[11]), int(data[17]), int(data[18]), float(data[8]), float(data[9]), data[12], data[13], MTflag, snapno 
					break

				if float(data[11]) == id0:
					if R0 > RL_Egg_0 or R1 > RL_Egg_1: 
						MTflag = 1
					...
					timearr.append(time)
					ID_FLAG = 0
					print time, 0, int(data[11]), int(data[10]), int(data[18]), int(data[17]), float(data[9]), float(data[8]), data[12], data[13], MTflag, snapno
					break
...
def NS_formation(id, path, string, snapno):
	id0_str = str(id)
        f = open('histories/NEW'+id0_str+'_history.dat','w')
        #sys.stdout = f
	...
	time_array = []
        ...
        for j in range(0,snapno,Delta):
                ...
                if j-Delta < snapno_max:
                        ...
                        f2_f = gzip.open(path+string+'.snap'+snapno_f+'.dat.gz','r')
                        lines2 = f2_f.readlines()
                        line2 = lines2[0]
                        parsed = line2.split('[')
                        parsed = parsed[0]
                        ...
                        snapno_max_str = snapno
                        f2_f = gzip.open(path+string+'.snap'+snapno_max_str+'.dat.gz','r')
                        lines2 = f2_f.readlines()
                        line2 = lines2[0]
                        parsed = line2.split('[')
                        parsed = parsed[0]
        ...
        ###################
			...
			if float(data[10]) == id:
				...
				M_comp = float(data[9])
				NS_ID = id
				comp_ID = float(data[11])
				k_NS = float(data[17])
				k_comp = float(data[18])
				a = float(data[12])
				ecc = float(data[13])
			if float(data[11]) == id:
				binflag = 1
                                ...
                                M_comp = float(data[8])
                                NS_ID = id
                                comp_ID = float(data[10])
				k_NS = float(data[18])
                                k_comp = float(data[17])
                                a = float(data[12])
                                ecc = float(data[13])
			print time, 0 , binflag, M, M_comp, k_NS, k_comp, NS_ID, comp_ID, a, ecc
			if k_NS == 13:
				break
