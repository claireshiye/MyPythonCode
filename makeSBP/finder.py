...
def velocity_dispersion(path,string, snapno,ALL=1):
	...
	if ALL == 1:
		f4 = open(path+string+'.snap'+snapno+'.vel_dispersion.dat','w')
	if ALL == 0:
		f4 = open(path+string+'.snap'+snapno+'.vel_dispersion_giants.dat','w')
	...
	for i in range(0,len(data)):
		...
		if ALL == 1:
			if data[i,1] <= 1e6:	# Looks at all stars, not a cut.
				...
				r_xyz = [data[i,15], data[i,16], data[i,17]]  #x,y,z components of r in pc
				v_xyz = [data[i,18], data[i,19], data[i,20]]  #x,y,z components of velocity in km/s
				count = count + 1
				if data[i,2] == 1:
				...
				R.append(r)
				Vr.append(data[i,20])		# Just use the z component as your LOS direction
		if ALL == 0:
			if data[i,2] ==1:
...
################################################
	...
	for i in range(0,len(Vr)):
		print>>f, array[i,0], array[i,1], array[i,2]	#Print each radius/LOS velocity pair in order of radial position	
...
def find_max_snap_time(path,string):
        snapno = find_max_snap(path,string)
        units=scripts.read_units(path+string)
        km = units[0]['l_cgs']*1.0e-5
...
def find_vband_luminosity(path,snapno):
	string = 'initial'

	f = gzip.open(path+string+'.snap'+snapno+'.dat.gz','r')
	lines = f.readlines()

	f2 = open(path+string+'.snap'+snapno+'_vband.dat','w')
	...
	for i in range(2,len(lines)):
		...
		if np.float(data[7]) == 1:
			L1 = np.float(data[19])
			L2 = np.float(data[20])
			R1 = np.float(data[21])
			R2 = np.float(data[22])
			L1 = conversions.V_band_Lum_calculator(R1,L1)
			L2 = conversions.V_band_Lum_calculator(R2,L2)
			L = L1 + L2
		if np.float(data[7]) != 1:
			L = np.float(data[15])
			R = np.float(data[16])
			L_sun = conversions.V_band_Lum_calculator(1,1)
			L = conversions.V_band_Lum_calculator(R,L)/L_sun
