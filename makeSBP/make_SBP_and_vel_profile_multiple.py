...
pathlist = np.genfromtxt('/projects/b1095/syr904/cmc/cmc-mpi-tidalcapture/rvgrid/path_rvgrid.dat', dtype=str)
N=np.full_like(len(pathlist), 800000.)
print(N)
Z=np.full_like(len(pathlist), 0.001)
rv=[0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.5, 2.0, 3.0, 5.0]
rg=np.full_like(len(pathlist), 8)

for kk in range(len(pathlist)):
    path = pathlist[kk]
    print(path)
    string = 'initial'
